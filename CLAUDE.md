...
- **`docs/graphql-guide.md`** — Guide to the generated GlideRecord schema in `/api/now/graphql` as an alternative to the REST Table API: `GlideRecord_Query`/`_Mutation`, encoded queries, dot-walking via `_reference`, inline schema/ACL metadata, aggregates, journal-field access, mutation caveats (writes can return null counts — verify by re-query), and in-band errors (HTTP 200 + `errors[]`). Source of the built guide page.
...
- **`docs/catalog-variables.md`** — Updating catalog variables on tasks/RITMs via the undocumented `PUT /api/sn_sc/servicecatalog/variables/{table_name}/{sys_id}` (works for itil users; Table API writes to `sc_item_option` 403 for them). Verified live; covers the flat body shape, the silent-no-op failure modes, and record-producer (`question_answer`) variables.
...
- **`sn-graphql-guide.html`** — Rendered page for `docs/graphql-guide.md`. Build artifact: never hand-edit; edit the markdown and regenerate with `npm run build:guide` (source: `src/build-guide.js`, dependency-free like the other builders — CI runs it without npm install).
